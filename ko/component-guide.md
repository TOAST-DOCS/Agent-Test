...
| log_timestamps | MySQL 5.7의 기본 `log` 시간은 UTC로 표시됩니다. 로그 시간을 `SYSTEM` 로컬 시간으로 변경합니다. | 부분 지원 |
| slow\_query\_log | slow\_query log 옵션을 사용합니다\. long\_query\_time에 따른 기본 10초 이상의 쿼리는 slow\_query\_log에 기록됩니다\. | O |
| sysdate-is-now | `sysdate`의 경우 `replication`에서 `sysdate()` 사용된 SQL문은 복제 시 마스터와 슬레이브 간의 시간이 달라지는 문제가 있어 `sysdate()`와 `now()`의 함수를 동일하게 적용합니다. | 부분 지원 |
