...
このドキュメントは、mkdocs-macros の Jinja 条件付き分岐と変数置換が ko/en/ja 三つの言語にわたって同一に維持されているかを検証するためのフィクスチャです。タグは制御構文であるため翻訳されず、本文のみ言語別に異なります。(本文修正テスト: この文は翻訳を再実行する際に反映されるべきです。)
