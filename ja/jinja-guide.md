...
このドキュメントは、mkdocs-macros の Jinja 条件分岐と変数置換が ko/en/ja 3言語で同じように維持されているかを検証するためのフィクスチャーです。タグは制御構文であるため翻訳されず、本文のみ言語ごとに異なります。
{%- endif %}
...
政府ネットワーク環境では専用エンドポイントを使用します。公開ドメインではアクセスできません。（タグブロック内のテキスト編集：この文は再翻訳時に反映される必要があります。）
{% else -%}
公開環境ではデフォルトエンドポイントを使用します。リージョン別ホストについては、以下の表を参照してください。
{% endif %}
