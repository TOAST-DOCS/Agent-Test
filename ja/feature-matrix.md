...
### 自動 ID 割り当て検証用の新しいサブセクション { #new-subsection-for-validating-automatic-id-assignment }

このサブセクションは、翻訳パイプラインの anchor-id の自動割り当てを検証するための新しい h3 です。ko の変更時に anchor id を付けず、翻訳ジョブが ko/en/ja の 3 言語に同一の id を割り当てる必要があります。
...
- **インスタンス作成**: イメージとタイプを選択して仮想サーバーを作成します。(リスト修正テスト)
- **インスタンステンプレート**: よく使う設定をテンプレートとして保存して再利用します。
- **スケジューリング**: 指定した時間にインスタンスを開始または停止します。
- **モニタリング**: CPU、メモリ、ディスクの使用量をダッシュボードで確認します。
...
### 料金プランごとの提供上限および詳細 { #feature-by-plan }
