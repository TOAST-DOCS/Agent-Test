...
## 自動 ID 割り当て検証用の新規セクション { #new-section-for-validating-automatic-id-assignment }

このセクションは、翻訳パイプラインの anchor-id 自動割り当てを検証するための新規セクションです。ko の変更時に anchor-id を付加しない場合、翻訳ジョブが ko/en/ja 3 つの言語に対して同じ ID を割り当てる必要があります。
