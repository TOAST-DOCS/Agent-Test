...
このドキュメントは**リンク修正**(`dashboard/links/fix.py`, Jenkins `fix-links`) e2e フィクスチャーです。(順序テストA: この文は lag-order-a-edit 検証用です。)
以下のセクションでは、決定的なルール1つずつを対象とした**意図的に誤って書かれたリンク**を含んでおり、
`scripts/e2e-fix-links.sh` が修正結果をルール別に判定します。
