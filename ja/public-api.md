...
| `MIGRATING` | インスタンスがマイグレーション中の場合<br/>ライブマイグレーション(アクティブインスタンスの移動)操作中に発生します |
...
| `UNKNOWN` | インスタンスの状態が不明な場合<br/>インスタンスがこの状態に進入した場合、管理者に問い合わせます。 |
...
| marker | Query | UUID | - | リストの最初のインスタンス UUID<br/>ソート基準に従って、`marker` で指定されたインスタンスから `limit` 個のインスタンス一覧を返します。 |
...
| server.addresses | Body | Object | インスタンス IP リスト オブジェクト <br/>インスタンスに接続されたポート数だけリストが生成されます。 |
...
| server.security_groups | Body | Object | - | セキュリティグループのリスト<br/>省略した場合、`default` グループが追加されます |
| server.security_groups.name | body | String | - | **(条件付き必須)** インスタンスに追加するセキュリティグループ名 |
| server.user_data | Body | String | - | インスタンスの起動後に実行するスクリプトおよび設定<br/>Base64 エンコード文字列として 65535 バイトまで |
| server.availability_zone | Body | String | - | インスタンスを作成する可用性ゾーン。<br/>指定しない場合、ランダムに選択されます。<br/>ルートブロックストレージのソースタイプが `volume`、`snapshot` である場合、ソースブロックストレージの可用性ゾーンと同じに設定する必要があります。 |
...
| server.return_reservation_id | Body | Boolean | - | インスタンス作成リクエストの予約ID。<br/>`true` に設定すると、インスタンス作成情報の代わりに予約ID を返します。<br/>デフォルト値は `false` です。 |
