...
| `REBOOT` | インスタンスがソフト再起動状態の場合<br/>再起動コマンドが仮想マシン オペレーティングシステムに送信されます |
...
| `UNKNOWN` | インスタンスの状態を確認できない場合<br/>インスタンスがこの状態になった場合は、管理者にお問い合わせください。 |
...
| limit | Query | Integer | - | インスタンスリストの件数<br/>指定された件数分のインスタンスリストを返す |
| marker | Query | UUID | - | 目録の最初のインスタンス UUID<br/>ソート基準に従って、`marker` で指定されたインスタンスから `limit` 個数分のインスタンス目録を返します。 |
...
| servers.addresses | Body | Object | インスタンス IP リストオブジェクト。<br/>インスタンスに接続されたポートの数だけリストが生成されます。 |
...
| server.addresses | Body | Object | インスタンス IP リスト オブジェクト <br/>インスタンスに接続されたポート数分のリストが作成されます                                                                                                                                                              |
...
| server.availability_zone | Body | String | - | インスタンスを作成する可用性ゾーン<br/>指定しない場合は任意に選択されます<br/>ルートブロックストレージのソースタイプが `volume`、`snapshot` の場合、ソースブロックストレージの可用性ゾーンと同じに設定する必要があります |
...
| server.return_reservation_id | Body | Boolean | - | インスタンス作成リクエストの予約 ID。<br/>`True` に指定するとインスタンス作成情報の代わりに予約 ID を返す。<br/>デフォルト値は `False` |
