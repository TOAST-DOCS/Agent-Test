...
インスタンスは、仮想 CPU、メモリ、ルートブロックストレージで構成された仮想サーバーです。このサーバーに顧客のサービスまたはアプリケーションをインストールし、NHN Cloud が提供するさまざまなサービスと組み合わせて使用します。（同時 PR テスト A：この文は concurrent-a-edit 検証用です。）
...
| x9c | 並行PR テスト用に追加したタイプです。実際のサービスには存在しません。 |
...
Mac または Linux には、通常 SSH クライアントがデフォルトでインストールされています。SSH クライアントで以下のようにキーペアの秘密鍵を使用して接続します。
...
ブート失敗やネットワーク構成問題など、SSH クライアントが使用できない状況では、シリアルコンソールに接続してインスタンスにアクセスできます。

シリアルコンソール機能には、以下の制限事項があります。

* インスタンスあたり 1 つのシリアルコンソール接続のみが可能であり、複数接続の試行時に正常に接続できない場合があります。
* ユーザーがアップロードしたイメージで作成したインスタンスおよびプライベートイメージで作成したインスタンスでは、シリアルコンソール接続を保証しません。
* シリアルコンソール接続は最大 10 分間接続できます。
* Windows インスタンスはシリアルコンソール機能をサポートしていません。
* 2026 年 1 月 27 日のデプロイメント前に作成したインスタンスは、**[インスタンス停止]**後に**[インスタンス開始]**が必要です。**[インスタンス再起動]**機能では適用されません。

> [注意]
> シリアルコンソール経由でインスタンスに接続してブート方法を変更した場合、ブートが失敗する可能性があります。その結果に対する責任はユーザーにあります。
> 一般的な状況では、SSH クライアント接続の使用をお勧めします。

#### GRUB ブートローダー設定の変更

2024年11月26日のデプロイ前に作成したインスタンスでブートローダーを操作するには、GRUB 設定が必要です。
...
変更された設定を適用します。OS によって GRUB 設定を適用するコマンドが異なる場合があります。
...
このセクションは、同時 PR シナリオの検証のため PR B が追加したセクションです。PR A の翻訳がこのセクションの翻訳を削除してはいけません。
