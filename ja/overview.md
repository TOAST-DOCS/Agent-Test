...
インスタンスは仮想のCPU、メモリ、ルートブロックストレージで構成された仮想サーバーです。このサーバーに顧客のサービスやアプリケーションをインストールしてNHN Cloudが提供する様々なサービスを組み合わせて使用します。 (同時PR テストA: この文章は concurrent-a-edit 検証用です。)
...
| x9c | 同時PRテスト用に追加された仕様です。実際のサービスには存在しません。 |
...
このセクションは同時PRシナリオ検証のためにPR B が追加したセクションです。PR A の翻訳がこのセクションの翻訳を削除してはいけません。
