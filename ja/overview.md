...
インスタンスは仮想の CPU、メモリ、ルート ブロックストレージで構成される仮想サーバーです。このサーバーに顧客のサービスまたはアプリケーションをインストールし、NHN Cloud が提供するさまざまなサービスを組み合わせて使用します。(本文変更テスト：この文は再翻訳時に反映する必要があります。)
...
このセクションは翻訳パイプラインテスト用に新たに追加したセクションです。新規セクションが翻訳され、ko/en/ja に同一の anchor id が付与されるかを確認します。
