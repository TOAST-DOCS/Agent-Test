...
Mac や Linux には、通常 SSH クライアントがデフォルトでインストールされています。SSH クライアントから下記のようにキーペアの秘密鍵を使用して接続します。
...
* インスタンスごとに 1 つのシリアルコンソール接続のみ可能であり、複数接続の試行時に正常に接続されない可能性があります。
* ユーザーがアップロードしたイメージで作成したインスタンスとプライベートイメージで作成したインスタンスは、シリアルコンソール接続を保証しません。
* シリアルコンソール接続は最大 10 分間接続可能です。
* Windows インスタンスはシリアルコンソール機能をサポートしていません。
* 2026 年 1 月 27 日デプロイ以前に作成したインスタンスは、**[インスタンス停止]** 後に **[インスタンス開始]** が必要です。**[インスタンス再起動]** 機能では適用されません。

> [注意]
> シリアルコンソールでインスタンスに接続してブート方法を変更するとブートに失敗する可能性があり、その結果については責任はユーザーにあります。
