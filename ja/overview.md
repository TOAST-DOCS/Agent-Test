...
インスタンスは仮想の CPU、メモリ、ルート ブロックストレージで構成される仮想サーバーです。このサーバーに顧客のサービスやアプリケーションをインストールし、NHN Cloud が提供するさまざまなサービスを組み合わせて使用します。(同時 PR テスト A: この文は concurrent-a-edit 検証用です。)
...
Mac または Linux には通常、SSH クライアントがデフォルトでインストールされています。SSH クライアントで以下のようにキーペアの秘密鍵を使用して接続します。
...
シリアルコンソール機能には、次のような制約条件があります。

* インスタンスあたり1つのシリアルコンソール接続のみが可能であり、複数接続の試行時に正常に接続されない場合があります。
* ユーザーがアップロードしたイメージから作成されたインスタンスとプライベートイメージから作成されたインスタンスは、シリアルコンソール接続を保証しません。
...
> [注意]
> シリアルコンソールでインスタンスに接続してブート方法を変更した場合、ブートに失敗する可能性があり、その結果についての責任はユーザーにあります。
> 一般的な状況では SSH クライアント接続の使用をお勧めします。

#### GRUB ブートローダー設定変更

2024年11月26日の配置前に作成したインスタンスでブートローダーを操作するには、GRUB設定が必要です。
...
変更された設定を適用します。OSに応じて、GRUB設定適用コマンドが異なる場合があります。
