...
Mac や Linux には通常、SSH クライアントがデフォルトでインストールされています。SSH クライアントで以下のようにキーペアの秘密鍵を使用して接続します。
...
シリアルコンソール機能には次の制限があります。

* インスタンスあたり 1 つのシリアルコンソール接続のみ可能であり、複数接続を試みた場合、正常に接続できない可能性があります。
* ユーザーがアップロードしたイメージから作成されたインスタンスとプライベートイメージから作成されたインスタンスは、シリアルコンソールアクセスを保証しません。
* シリアルコンソール接続は最大 10 分間アクセスできます。
* Windows インスタンスはシリアルコンソール機能をサポートしていません。
* 2026 年 1 月 27 日の展開前に作成されたインスタンスは、**[インスタンスを停止]** した後、**[インスタンスを開始]** する必要があります。**[インスタンスを再起動]** 機能では適用されません。

> [注意]
> シリアルコンソールでインスタンスにアクセスしてブート方法を変更する場合、ブートに失敗する可能性があります。その結果に対する責任はユーザーにあります。
