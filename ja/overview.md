...
インスタンスは、仮想 CPU、メモリ、ルートブロックストレージで構成された仮想サーバーです。このサーバーに顧客のサービスまたはアプリケーションをインストールし、NHN Cloud が提供するさまざまなサービスを組み合わせて使用します。(順序テスト A: このセンテンスは lag-order-a-edit 検証用です。)
...
Mac や Linux には通常、SSH クライアントがデフォルトでインストールされています。SSH クライアントで以下のように、キーペアの秘密鍵を使用して接続します。
...
* インスタンスあたり 1 つのシリアルコンソール接続のみ可能です。複数の接続を試みると、正常に接続できない場合があります。
* ユーザーがアップロードしたイメージで作成されたインスタンスと個人イメージで作成されたインスタンスは、シリアルコンソール接続を保証しません。
* シリアルコンソール接続は最大 10 分間アクセス可能です。
* Windows インスタンスはシリアルコンソール機能をサポートしていません。
* 2026 年 1 月 27 日のデプロイ前に作成されたインスタンスは、**[インスタンス中止]** の後に **[インスタンス起動]** が必要です。**[インスタンス再起動]** 機能では適用されません。

> [注意]
> シリアルコンソールでインスタンスに接続して起動方法を変更する場合、起動に失敗する可能性があり、その結果に対する責任はユーザーにあります。
> 一般的な状況では、SSH クライアント接続の使用をお勧めします。

#### GRUB ブートローダー設定の変更

2024年11月26日のデプロイ前に作成したインスタンスでブートローダーを操作するには、GRUB設定が必要です。
...
変更された設定を適用します。OSに応じて、GRUB設定の適用コマンドが異なる場合があります。
