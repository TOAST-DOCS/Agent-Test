...
* インスタンスごとに 1 つのシリアルコンソール接続のみ可能であり、複数のアクセスを試みた場合、正常に接続されない可能性があります。
* ユーザーがアップロードしたイメージで作成されたインスタンスおよびプライベートイメージで作成されたインスタンスは、シリアルコンソール接続を保証しません。
* シリアルコンソール接続は、最大 10 分間アクセス可能です。
* Windows インスタンスはシリアルコンソール機能をサポートしていません。
* 2026 年 1 月 27 日の配布前に作成されたインスタンスは、**[インスタンス停止]** の後に **[インスタンス起動]** が必要です。**[インスタンス再起動]** 機能では適用されません。

> [注意]
> シリアルコンソールでインスタンスにアクセスしてブート方法を変更すると、ブートに失敗する可能性があり、その結果について責任はユーザーにあります。
...
変更された設定を適用します。OS に応じて GRUB 設定を適用するコマンドが異なる場合があります。
...
このセクションは PR B が追加しました。PR A の翻訳ジョブはこのセクションに触れてはいけず、PR B の翻訳 PR がマージされた場合、en/ja に一度だけ存在する必要があります。
