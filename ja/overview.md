...
インスタンスは、仮想の CPU、メモリ、ルートブロックストレージで構成された仮想サーバーです。このサーバーに顧客のサービスやアプリケーションをインストールし、NHN Cloud が提供するさまざまなサービスを組み合わせて使用します。（本文修正テスト：この文は翻訳再実行時に反映される必要があります。）
...
このセクションは、翻訳パイプラインテストのために新たに追加されたセクションです。新規セクションが翻訳され、ko/en/ja に同じアンカーID が付与されるかどうかを確認します。
