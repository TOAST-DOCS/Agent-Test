...
インスタンスは仮想の CPU、メモリ、ルートブロックストレージで構成される仮想サーバーです。このサーバーに顧客のサービスやアプリケーションをインストールし、NHN Cloud が提供するさまざまなサービスを組み合わせて使用します。(concurrent PR テスト A: この文は concurrent-a-edit 検証用です。)
...
| x9c | 並行 PR テスト用に追加されたタイプです。実際のサービスには存在しません。 |
...
Mac や Linux には、通常 SSH クライアントがデフォルトでインストールされています。SSH クライアントから、以下のようにキーペアの秘密鍵を使用して接続します。
...
シリアルコンソール機能には次の制約条件があります。

* インスタンスあたり 1 つのシリアルコンソール接続のみ可能であり、複数接続を試みた場合、正常に接続されない可能性があります。
* 個人がアップロードしたイメージで作成したインスタンスおよびプライベートイメージで作成したインスタンスは、シリアルコンソール接続を保証しません。
* シリアルコンソール接続の最大接続時間は 10 分です。
* Windows インスタンスはシリアルコンソール機能をサポートしていません。
* 2026 年 1 月 27 日の配布より前に作成したインスタンスは、**[インスタンス停止]** の後に **[インスタンス開始]** が必要です。**[インスタンス再起動]** 機能では適用されません。

> [注意]
> シリアルコンソールでインスタンスに接続してブート方法を変更した場合、ブートに失敗する可能性があり、その結果に対する責任はユーザーにあります。
> 通常の状況では、SSH クライアント接続の使用をお勧めします。

#### GRUB ブートローダー設定変更

2024年11月26日デプロイ以前に作成したインスタンスでブートローダーを操作するには、GRUB 設定が必要です。
...
変更された設定を適用します。OS によって GRUB 設定適用コマンドが異なる場合があります。
...
## 同時PR テスト追加セクション { #concurrent-b-added }

このセクションは、同時PR シナリオ検証のために PR B が追加したセクションです。PR A の翻訳がこのセクションの翻訳を削除してはいけません。
