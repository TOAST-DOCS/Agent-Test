...
インスタンスは、仮想 CPU、メモリ、ルート ブロックストレージで構成される仮想サーバーです。このサーバーに顧客のサービスやアプリケーションをインストールし、NHN Cloud が提供するさまざまなサービスを組み合わせて使用します。(同時 PR テスト A: この文は concurrent-a-edit 検証用です。)
...
Mac または Linux には通常、SSH クライアントがデフォルトでインストールされています。SSH クライアントから以下のようにキーペアの秘密鍵を使用して接続します。
...
ブート失敗やネットワーク構成の問題など、SSHクライアントを使用できない状況では、シリアルコンソールに接続してインスタンスにアクセスできます。

シリアルコンソール機能には、次のような制約があります。

* インスタンスごとに1つのシリアルコンソール接続のみ可能です。複数の接続試行時には、正常に接続されない場合があります。
* ユーザーがアップロードしたイメージで作成されたインスタンスおよびプライベートイメージで作成されたインスタンスは、シリアルコンソール接続を保証しません。
* シリアルコンソール接続は、最大10分間アクセス可能です。
* Windowsインスタンスはシリアルコンソール機能をサポートしていません。
* 2026年1月27日のデプロイ前に作成されたインスタンスは、**インスタンス停止**の後に**インスタンス開始**が必要です。**インスタンス再起動**機能では適用されません。

> [注意]
> シリアルコンソールでインスタンスにアクセスしてブート方法を変更する場合、ブートが失敗する可能性があります。その結果に対する責任はユーザーにあります。
> 通常の状況では、SSHクライアント接続の使用をお勧めします。

#### GRUB ブートローダー設定変更

2024年11月26日の配布前に作成したインスタンスでブートローダーを操作するには、GRUB 設定が必要です。
...
変更された設定を適用します。OS に応じて GRUB 設定適用コマンドが異なる場合があります。
...
## 並行 PR テスト用追加セクション { #concurrent-b-added }

このセクションは、並行 PR シナリオの検証のため PR B が追加したセクションです。PR A の翻訳がこのセクションの翻訳を削除してはいけません。
