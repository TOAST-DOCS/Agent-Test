...
インスタンスは仮想 CPU、メモリ、ルートブロックストレージで構成される仮想サーバーです。このサーバーに顧客のサービスやアプリケーションをインストールし、NHN Cloud が提供するさまざまなサービスを組み合わせて使用します。(同時 PR テスト A: この文は concurrent-a-edit 検証用です。)
...
ブート失敗、ネットワーク構成の問題など、SSH クライアントを使用できない状況でシリアルコンソールに接続し、インスタンスにアクセスできます。

シリアルコンソール機能には、次の制約条件があります。

* インスタンスあたり 1 つのシリアルコンソール接続のみが可能であり、複数接続を試みた場合、正常に接続されない可能性があります。
* ユーザーがアップロードしたイメージから作成されたインスタンスと個人イメージから作成されたインスタンスは、シリアルコンソールへのアクセスを保証しません。
...
> [注意]
> シリアルコンソールでインスタンスにアクセスしてブート方法を変更した場合、ブートに失敗する可能性があり、その結果について責任はユーザーにあります。
> 通常の状況では、SSH クライアント接続の使用をお勧めします。

#### GRUB ブートローダー設定の変更

2024年11月26日のデプロイ以前に作成したインスタンスでブートローダーを操作するには、GRUB 設定が必要です。
...
このセクションは、同時 PR シナリオの検証のために PR B が追加したセクションです。PR A の翻訳がこのセクションの翻訳を削除してはいけません。
