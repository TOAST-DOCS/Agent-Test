...
インスタンスは仮想のCPU、メモリ、ルートブロックストレージで構成された仮想サーバーです。このサーバーに顧客のサービスまたはアプリケーションをインストールし、NHN Cloudが提供するさまざまなサービスを組み合わせて使用します。（本文修正テスト：この文は翻訳再実行時に反映される必要があります。）
...
このセクションは翻訳パイプラインのテスト用に新しく追加されたセクションです。新しいセクションが翻訳され、ko/en/ja に同一の anchor id が付与されることを確認します。
