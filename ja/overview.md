...
Mac または Linux には、通常 SSH クライアントがデフォルトでインストールされています。SSH クライアントから、以下のようにキーペアの秘密鍵を使用して接続します。
...
シリアルコンソール機能は、次のような制約条件があります。

* インスタンスあたり 1 つのシリアルコンソール接続のみ可能で、複数接続の試行時に正常に接続されない可能性があります。
* ユーザーがアップロードしたイメージから作成したインスタンスおよびカスタムイメージから作成したインスタンスは、シリアルコンソールアクセスを保証しません。
* シリアルコンソール接続は最大 10 分間アクセス可能です。
* Windows インスタンスはシリアルコンソール機能をサポートしていません。
* 2026 年 1 月 27 日のデプロイメント前に作成したインスタンスは、**[インスタンス停止]** の後に **[インスタンス起動]** が必要です。**[インスタンス再起動]** 機能では適用されません。

> [注意]
> シリアルコンソール経由でインスタンスにアクセスしてブート方法を変更する場合、ブートに失敗する可能性があります。これに伴う結果に対する責任はユーザーにあります。
