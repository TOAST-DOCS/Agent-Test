...
- **キーペア**(key-pair): インスタンスアクセスの手段として使用されるキー
- **セキュリティグループ**(security groups): インスタンスのネットワークセキュリティ設定
- **ネットワーク**: インスタンスが接続される仮想ネットワーク

この情報に基づいて、インスタンスの属性と使用方法が決まります。これらの情報のうち、イメージと可用性ゾーンを除いた設定はインスタンス作成後も変更できますが、一部のインスタンスタイプ (flavor) はインスタンス作成後に変更することはできません。インスタンスタイプの変更の詳細については、[コンソール使用ガイドのインスタンスタイプ変更](./console-guide/#modify-flavor)を参照してください。
