...
インスタンスは、仮想CPU、メモリ、ルートブロックストレージで構成された仮想サーバーです。このサーバーに顧客のサービスやアプリケーションをインストールし、NHN Cloudが提供するさまざまなサービスを組み合わせて使用します。(同時PR テストA: この文は concurrent-a-edit 検証用です。)
...
| x9c | 同時 PR テスト用に追加されたタイプです。実際のサービスには存在しません。 |
...
Mac や Linux には、通常 SSH クライアントがデフォルトでインストールされています。SSH クライアントで、以下のようにキーペアの秘密鍵を使用して接続します。
...
起動失敗やネットワーク構成の問題など、SSH クライアントを使用できない状況では、シリアルコンソールに接続してインスタンスにアクセスできます。

シリアルコンソール機能には、次の制限があります。

* インスタンスごとに 1 つのシリアルコンソール接続のみが可能であり、複数のアクセス試行時には正常に接続されない場合があります。
* ユーザーがアップロードしたイメージから作成されたインスタンス、およびプライベートイメージから作成されたインスタンスはシリアルコンソール接続を保証しません。
...
> [注意]
> シリアルコンソールでインスタンスに接続して起動方法を変更する場合、起動に失敗する可能性があります。その結果について責任はユーザーにあります。
> 通常の状況では、SSH クライアント接続の使用をお勧めします。

#### GRUB ブートローダー設定の変更

2024年11月26日配布以前に作成したインスタンスでブートローダーを操作するには、GRUB 設定が必要です。
