...
インスタンスは、仮想の CPU、メモリ、ルートブロックストレージで構成された仮想サーバーです。このサーバーに顧客のサービスやアプリケーションをインストールし、NHN Cloud が提供するさまざまなサービスを組み合わせて使用します。インスタンスは数分で作成でき、必要に応じていつでも拡張または縮小できます。
...
- **タイプ**(flavor): インスタンスの仮想ハードウェアスペック
...
* 例: パブリックリージョンにのみ適用されるポリシー・価格・SLA 案内
...
- **バックアップ**: 指定されたスケジュールに従って、インスタンスのルートブロックストレージを定期的にコピーし、別のストレージに保管します。長期保管および災害復旧に適しています。
- **スナップショット**: 特定時点のインスタンスの状態をそのまま保存します。システム変更前のロールバックポイントとして使用するのに適しています。

バックアップとスナップショットは、元のインスタンスが削除された後も保持されるため、削除後もこれらを使用してインスタンスを復元できます。詳細については、「[バックアップサービス概要](/Storage/Backup/ja/overview/)」を参照してください。
...
```
$ sudo update-grub
<!-- heading-lint: F1 L257 — レビュー後にこの行を削除 (suggestion accept の際に自動削除) -->
```
