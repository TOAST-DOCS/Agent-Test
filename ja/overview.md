...
インスタンスは、仮想 CPU、メモリ、ルートブロックストレージで構成された仮想サーバーです。このサーバーにお客様のサービスやアプリケーションをインストールし、NHN Cloud が提供するさまざまなサービスを組み合わせて使用します。インスタンスは数分以内に作成でき、必要に応じていつでもスケールアップまたはスケールダウンできます。
...
- **キーペア** (key-pair): インスタンスへの接続手段として使用されるキー
...
このセクションは `this-is-only-public` フラグでビルドしたサイトでのみ表示されます。他のゾーン/環境のドキュメントには表示されません。

* 例: 共用リージョンにのみ適用されるポリシー・料金・SLAの案内
* 例: 商用デプロイ対象にのみ表示するプロモーション文言
<!-- @endif -->
...
インスタンスのデータを安全に保護するには、定期的なバックアップとスナップショットの作成を推奨します。バックアップとスナップショットには次のような違いがあります。

- **バックアップ(Backup)**: 指定されたスケジュールに従って、インスタンスのルートブロックストレージを定期的にコピーし、別のリポジトリに保管します。長期保管と災害復旧に適しています。
- **スナップショット(Snapshot)**: 特定の時点におけるインスタンスの状態をそのまま保存します。システム変更前のロールバックポイントとして使用するのに適しています。

バックアップとスナップショットは、元のインスタンスが削除されても保持されるため、削除後もこれらを使用してインスタンスを復元できます。詳細については、[バックアップサービス概要](/Storage/Backup/ja/overview/)を参照してください。
...
#### Mac または Linux の SSH クライアントから Linux インスタンスに接続する方法
...
#### WindowsでPuTTY SSHクライアントを使用してLinuxインスタンスに接続する方法
...
```
$ sudo update-grub
<!-- heading-lint: F1 L257 — レビュー後にこの行を削除してください（提案を承認すると自動削除されます） -->
```
