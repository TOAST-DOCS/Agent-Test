...
インスタンスは仮想の CPU、メモリ、ルートブロックストレージで構成された仮想サーバーです。このサーバーに顧客のサービスやアプリケーションをインストールし、NHN Cloud が提供する様々なサービスを組み合わせて使用します。(同時 PR テスト A：この文は concurrent-a-edit 検証用です。)
...
| x9c | 並行 PR テスト用に追加したタイプです。実際のサービスには存在しません。 |
...
Mac または Linux には、通常 SSH クライアントがデフォルトでインストールされています。SSH クライアントから、キーペアの秘密鍵を使用して以下のように接続します。
...
ブート失敗やネットワーク構成の問題など、SSH クライアントを使用できない状況でシリアルコンソールに接続してインスタンスにアクセスできます。

シリアルコンソール機能には、以下のような制限があります。

* インスタンスあたり 1 つのシリアルコンソール接続のみ可能であり、複数の接続試行時に正常に接続されない可能性があります。
* ユーザーがアップロードしたイメージから作成されたインスタンスおよびプライベートイメージから作成されたインスタンスは、シリアルコンソール接続を保証しません。
* シリアルコンソール接続は最大 10 分間接続できます。
* Windows インスタンスはシリアルコンソール機能をサポートしていません。
* 2026 年 1 月 27 日のデプロイ前に作成されたインスタンスは、**インスタンスの停止**後に**インスタンスの開始**が必要です。**インスタンスの再起動**機能では適用されません。

> [注意]
> シリアルコンソール経由でインスタンスにアクセスしてブート方法を変更する場合、ブートに失敗する可能性があり、その結果に対する責任はユーザーにあります。
> 一般的な状況では、SSH クライアント接続の使用をお勧めします。

#### GRUB ブートローダー設定の変更

2024年11月26日のデプロイ前に作成されたインスタンスでブートローダーを操作するには、GRUB設定が必要です。
...
変更した設定を適用します。OSに応じて、GRUB設定適用コマンドが異なる場合があります。
...
## 並行 PR テスト追加セクション { #concurrent-b-added }

このセクションは、並行 PR シナリオの検証のために PR B が追加したセクションです。PR A の翻訳がこのセクションの翻訳を削除してはいけません。
