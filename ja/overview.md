...
Mac または Linux には、通常 SSH クライアントがデフォルトでインストールされています。SSH クライアントから、以下のようにキーペアの秘密キーを使用して接続します。
...
ブート失敗やネットワーク構成の問題など、SSH クライアントを使用できない状況では、シリアルコンソールに接続し、インスタンスにアクセスできます。

シリアルコンソール機能には、次の制約条件があります。

* インスタンスあたり 1 つのシリアルコンソール接続のみが可能で、複数接続の試行時には正常に接続できない場合があります。
* ユーザーがアップロードしたイメージおよびプライベートイメージから作成されたインスタンスは、シリアルコンソール接続が保証されません。
* シリアルコンソール接続は最大 10 分間アクセスできます。
* Windows インスタンスはシリアルコンソール機能をサポートしていません。
* 2026 年 1 月 27 日のデプロイ前に作成したインスタンスは、**[インスタンスの停止]** の後に **[インスタンスの開始]** が必要です。**[インスタンスの再起動]** 機能では適用されません。

> [注意]
> シリアルコンソール経由でインスタンスに接続してブート方法を変更する場合、ブートに失敗する可能性があり、その結果に対する責任はユーザーにあります。
