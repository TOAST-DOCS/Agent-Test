...
- **キーペア**（key-pair）: インスタンスへの接続手段として使用するキー
...
* 例：共用リージョンにのみ適用されるポリシー・価格・SLAのご案内
...
- **バックアップ(Backup)**: 指定されたスケジュールに従い、インスタンスのルートブロックストレージを定期的にコピーして別のストレージに保管します。長期保管と障害復旧に適しています。
...
```
$ sudo update-grub
<!-- heading-lint: F1 L257 — レビュー後にこの行を削除（提案を承認すると自動削除） -->
```
