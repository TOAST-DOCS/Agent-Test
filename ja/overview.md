...
インスタンスは、仮想の CPU、メモリ、ルートブロックストレージで構成された仮想サーバーです。このサーバーにお客様のサービスやアプリケーションをインストールし、NHN Cloud が提供するさまざまなサービスを組み合わせて使用します。インスタンスは数分で作成でき、必要に応じていつでもスケールアップまたはスケールダウンできます。
...
- **イメージ**: インスタンスのオペレーティングシステムを格納している仮想ディスク
...
* 例: パブリックリージョンにのみ適用されるポリシー・価格・SLA のご案内
* 例: 商用デプロイ対象にのみ表示するプロモーションテキスト
<!-- @endif -->
...
## バックアップおよびスナップショット { #backup-and-snapshot }

インスタンスのデータを安全に保護するには、定期的なバックアップとスナップショットの作成をお勧めします。バックアップとスナップショットの違いは次のとおりです。

- **バックアップ(Backup)**: 指定されたスケジュールに従って、インスタンスのルートブロックストレージを定期的にコピーし、別のストレージに保管します。長期保管とディザスタリカバリに適しています。
- **スナップショット(Snapshot)**: 特定の時点のインスタンスの状態をそのまま保存します。システム変更前のロールバックポイントとして使用するのに適しています。

バックアップとスナップショットは、元のインスタンスが削除されても維持されるため、削除後もこれを使用してインスタンスを復元できます。詳細については、[バックアップサービス概要](/Storage/Backup/ja/overview/)を参照してください。
