...
インスタンスは仮想の CPU、メモリ、ルートブロックストレージで構成された仮想サーバーです。このサーバーに顧客のサービスやアプリケーションをインストールし、NHN Cloud が提供するさまざまなサービスを組み合わせて使用します。(concurrent PR test A: この文は concurrent-a-edit 検証用です。)
...
| x9c | 並行PR テスト用に追加されたタイプです。実際のサービスには存在しません。 |
...
Mac や Linux には通常、SSH クライアントがデフォルトでインストールされています。SSH クライアントから以下のようにキーペアの秘密鍵を使用して接続します。
...
ブート失敗、ネットワーク構成の問題など、SSH クライアントを使用できない状況で、シリアルコンソールに接続してインスタンスにアクセスできます。

シリアルコンソール機能には、次の制約があります。

* インスタンスごとに 1 つのシリアルコンソール接続のみ可能です。複数のアクセス試行の場合、正常に接続されない可能性があります。
* 個人がアップロードしたイメージから作成されたインスタンス、およびプライベートイメージから作成されたインスタンスは、シリアルコンソールアクセスを保証しません。
...
> [注意]
> シリアルコンソールでインスタンスにアクセスしてブート方法を変更する場合、ブートに失敗する可能性があります。その結果について責任はユーザーにあります。
> 通常の状況では、SSH クライアント接続の使用をお勧めします。

#### GRUBブートローダーの設定変更

2024年11月26日配布以前に作成したインスタンスでブートローダーを操作するには、GRUB設定が必要です。
...
変更された設定を適用します。OSによって、GRUB設定の適用コマンドが異なる場合があります。
