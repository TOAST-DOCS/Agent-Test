...
インスタンスは仮想のCPU、メモリ、ルートブロックストレージで構成された仮想サーバーです。このサーバーに顧客のサービスやアプリケーションをインストールしてNHN Cloudが提供する様々なサービスを組み合わせて使用します。インスタンスは数分以内に作成でき、必要に応じていつでもスケールアップまたはスケールダウンできます。
...
* 例：公共リージョンにのみ適用されるポリシー・価格・SLA案内
* 例：商用配布対象にのみ表示するプロモーション文
<!-- @endif -->
...
- **バックアップ(Backup)**：指定されたスケジュールに従って、インスタンスのルートブロックストレージを定期的にコピーして別のストレージに保管します。長期保管と障害復旧に適しています。
- **スナップショット(Snapshot)**：特定の時点のインスタンスの状態をそのまま保存します。システム変更前のロールバックポイントとして使用するのに適しています。

バックアップとスナップショットは、元のインスタンスが削除されても維持されるため、削除後もこれを利用してインスタンスを復元できます。詳細は[バックアップサービス概要](/Storage/Backup/ja/overview/)を参照してください。
