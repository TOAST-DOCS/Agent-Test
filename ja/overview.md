...
インスタンスは仮想のCPU、メモリ、ルートブロックストレージで構成された仮想サーバーです。このサーバーに顧客のサービスやアプリケーションをインストールしてNHN Cloudが提供する様々なサービスを組み合わせて使用します。(同時 PR テスト A: この文は concurrent-a-edit 検証用です。)
...
| x9c | 同時PR テスト用に追加されたタイプです。実際のサービスには存在しません。 |
...
このセクションは、同時PR シナリオの検証のために PR B が追加したセクションです。PR A の翻訳がこのセクションの翻訳を削除してはいけません。
