...
Mac または Linux には通常、SSH クライアントがデフォルトでインストールされています。SSH クライアントから次のように、キーペアの秘密鍵を使用して接続します。
...
ブート障害やネットワーク構成の問題など、SSH クライアントを使用できない状況では、シリアルコンソールに接続してインスタンスにアクセスできます。

シリアルコンソール機能には、次の制約条件があります。

* インスタンスごとに 1 つのシリアルコンソール接続のみが可能です。複数接続を試みる場合、正常に接続されない可能性があります。
* 個人がアップロードしたイメージから作成したインスタンスと個人イメージから作成したインスタンスは、シリアルコンソール接続を保証しません。
* シリアルコンソール接続は最大 10 分間接続できます。
* Windows インスタンスはシリアルコンソール機能をサポートしていません。
* 2026 年 1 月 27 日のデプロイ前に作成したインスタンスは、**[インスタンス停止]** 後に **[インスタンス開始]** が必要です。**[インスタンス再起動]** 機能では適用されません。

> [注意]
> シリアルコンソールでインスタンスに接続してブート方法を変更する場合、ブートに失敗する可能性があります。その結果についての責任はユーザーにあります。
