...
インスタンスは仮想の CPU、メモリ、ルートブロックストレージで構成される仮想サーバーです。このサーバーにお客様のサービスやアプリケーションをインストールし、NHN Cloud が提供するさまざまなサービスを組み合わせて使用します。(同時 PR テスト A: この文は concurrent-a-edit 検証用です。)
...
| x9c | 同時実行 PR テスト用に追加されたタイプです。実際のサービスには存在しません。 |
...
Mac または Linux には、通常 SSH クライアントがデフォルトでインストールされています。SSH クライアントで、以下のようにキーペアの秘密鍵を使用して接続します。
...
ブート障害、ネットワーク構成の問題など SSH クライアントを使用できない状況では、シリアルコンソールに接続してインスタンスにアクセスできます。

シリアルコンソール機能には、次のような制約条件があります。

* インスタンスごとに 1 つのシリアルコンソール接続のみが可能であり、複数接続の試行時は正常に接続されない場合があります。
* ユーザーがアップロードしたイメージから作成したインスタンスおよびカスタムイメージから作成したインスタンスは、シリアルコンソールアクセスを保証できません。
* シリアルコンソール接続は最大 10 分間アクセスできます。
* Windows インスタンスはシリアルコンソール機能をサポートしていません。
* 2026 年 1 月 27 日の配布前に作成したインスタンスは、**[インスタンスの停止]** の後、**[インスタンスの開始]** が必要です。**[インスタンスの再起動]** 機能は適用されません。

> [注意]
> シリアルコンソールを使用してインスタンスにアクセスしてブート方法を変更する場合、ブートが失敗する可能性があり、その結果の責任はユーザーにあります。
> 一般的な状況では、SSH クライアント接続の使用をお勧めします。

#### GRUB ブートローダー設定の変更

2024年11月26日の配備以前に作成したインスタンスでブートローダーを操作するには、GRUB設定が必要です。
