...
インスタンスは、仮想の CPU、メモリ、ルートブロックストレージで構成された仮想サーバーです。このサーバーにお客様のサービスやアプリケーションをインストールし、NHN Cloud が提供するさまざまなサービスを組み合わせて使用します。インスタンスは数分で作成でき、必要に応じていつでも拡張または縮小できます。
...
- **タイプ**(flavor): インスタンスの仮想ハードウェアの性能
- **アベイラビリティーゾーン**(AZ, availability zone): インスタンスが作成される物理的な場所
...
* 例：パブリックリージョンにのみ適用されるポリシー・料金・SLA の案内
* 例：商用デプロイ対象にのみ表示するプロモーション文句
<!-- @endif -->
...
インスタンスのデータを安全に保護するには、定期的なバックアップとスナップショットの作成をお勧めします。バックアップとスナップショットには、次のような違いがあります。

- **Backup**: 指定されたスケジュールに従って、インスタンスのルートブロックストレージを定期的にコピーし、別のストレージに保管します。長期保管とディザスタリカバリに適しています。
- **Snapshot**: 特定の時点のインスタンスの状態をそのまま保存します。システム変更前のロールバックポイントとして使用するのに適しています。

バックアップとスナップショットは、元のインスタンスが削除された場合でも保持されるため、削除後もこれを使用してインスタンスを復元できます。詳細については、[バックアップサービス概要](/Storage/Backup/ja/overview/)を参照してください。
...
```
$ sudo update-grub
<!-- heading-lint: F1 L257 — レビュー後にこの行を削除 (提案を承認すると自動的に削除されます) -->
```
