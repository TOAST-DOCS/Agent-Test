...
本文に表があるbody stubです。入力後も、表の列数と行数がkoと同じである必要があります。
...
ブロック外のこの文だけが翻訳対象であり、コマンドとコメント行は変更されません。

<a id="fill-stub-heading"></a>
## 見出しまで空白のセクション { #fill-stub-heading }

en/ja の同じセクションは、見出しがまだ韓国語である見出しスタブです。
埋め込み実行は見出しと本文を一緒に翻訳しますが、見出しのレベルと `{ #id }` は ko に従う必要があります。

<a id="fill-stub-heading-child"></a>
### 下位の見出しも空白の場合 { #fill-stub-heading-child }

見出しスタブが連続して現れる場合です。親セクションとは独立して、それぞれ入力される必要があり、`###` レベルが `##` に昇格または降格されてはいけません。
