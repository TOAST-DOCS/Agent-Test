...
### セキュリティグループ TCP ポート 3389 (RDP) の許可 { #allow-security-group-tcp-port-3389-rdp }
...
| 名前 | 説明 | 対応状況 |
| --- | --- | --- |
| innodb_log_file_size | トランザクション (transaction) の redo ログを保存するログファイルのサイズを指定します。<br><br>実運用環境では 256 MB 以上をお勧めします。現在 512 MB に設定されています。設定値を修正する場合、DB の再起動が必要です。 | O |
...
| innodb\_file\_per\_table | テーブルが削除されるか `TRUNCATE` される場合、テーブル スペースは OS に直ちに返却されます。 | 部分サポート |
| innodb\_log\_files\_in\_group | innodb\_log\_fileファイルの個数を設定し、循環的\(circular\)に使用されます。最小2個以上で構成されます。 | O |
| log_timestamps | MySQL 5.7 のデフォルトログ時刻は `UTC` で表示されます。そのため、ログ時刻を `SYSTEM` ローカル時刻に変更します。 | 部分サポート |
| slow\_query\_log | slow\_query logオプションを使用します。 long\_query\_timeによる基本10秒以上のクエリーはslow\_query\_logに記録されます。 | O |
| sysdate-is-now | sysdate の場合、レプリケーション内で `sysdate()` が使用された SQL ステートメントは、複製時にマスターとスレーブ間の時間が異なる問題があるため、`sysdate()` と `now()` の関数を同じように適用します。 | 部分サポート |
