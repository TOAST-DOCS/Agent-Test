...
この段落は既存のセクションに追加されたテスト段落です。既存の見出しはそのまま維持する必要があります。

<a id="test-added-subsection"></a>
### テスト用下位セクション { #test-added-subsection }

この下位セクションは翻訳パイプラインのテスト用に追加されました。新規h3が翻訳され、3言語で同じアンカーIDが付与されるかを確認します。
...
### セキュリティグループ TCP ポート 3389 (RDP) を許可します { #allow-security-group-tcp-port-3389-rdp }
