...
このパラグラフは既存セクションに追加されたテストパラグラフです。既存のヘディングはそのままに維持される必要があります。

<a id="test-added-subsection"></a>
### テスト用のサブセクション { #test-added-subsection }

このサブセクションは翻訳パイプラインテストのために追加されました。新しい h3 が翻訳され、3つの言語に同一の anchor id が付与されるかどうかを確認します。
