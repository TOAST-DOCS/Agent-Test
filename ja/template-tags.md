...
{# 上記のマクロはレスポンステーブルの共通行を生成する — コメントはレンダリングされない #}

<a id="sample-template-tags"></a>
## Sample > テンプレートタグの形式一覧 { #sample-template-tags }

このドキュメントは、mkdocs-macros テンプレートタグが翻訳後もビルドされ、タグ内に著者が挿入した**韓国語文字列**が翻訳されるかどうかを検証するためのフィクスチャーです。`$[ product_name ]$` ドキュメントのタグは、実際のユーザーガイド（Private-DNS、Storage-Object-Storage、Storage-Online-NAS、nhn-cloud-foundry）からそのまま取り込んだ形式です。
...
!!! note "注記"
    ...
    オブジェクトロックコンテナはアーカイブコンテナ$[ " またはレプリケーション対象コンテナとして" if replication else "として" ]$指定することはできません。この制限を解除することはできません。

<a id="tt-macro-arg"></a>
### マクロの引数として渡された韓国語文字列 { #tt-macro-arg }

以下のテーブルの行はマクロが生成します。2 番目の引数は説明の前に付く接頭語です。
...
| 項目 | 必須 | 説明 |
|---|---|---|
| ユーザープロンプトテンプレート | X | 行ごとの入力値構成テンプレート。{{ カラム名 }} パターンが該当カラム値に置換され、設定すると結合区切り文字より優先して適用されます。カラム名は大文字と小文字を区別します。 |
...
共用環境では、コンソールから直接設定できます。この段落は開始タグが直上に付いているため、増分翻訳でタグと 1 つのユニットになります。設定は保存後すぐに適用されます。
{% endif %}
...
以下のブロックは韓国語がない対照群です。翻訳後もバイト単位で同じである必要があります。
