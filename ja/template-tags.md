...
| 項目 | 必須 | 説明 |
|---|---|---|
| ユーザープロンプトテンプレート | X | 行ごとの入力値を構成するテンプレートです。`{{ カラム名 }}` パターンが該当するカラムの値に置換されます。設定した場合、結合区切り文字より優先して適用されます。カラム名は大文字と小文字を区別します。 |
...
共有環境では、コンソールから直接設定できます。この段落は、直上に開始タグが付いており、増分翻訳においてタグと一つのユニットになります。設定は保存後すぐに適用されます。
{% endif %}
