...
## Sample > テンプレートタグの形式集 { #sample-template-tags }

このドキュメントは、mkdocs-macros テンプレートタグが翻訳後もビルドされ、タグ内に著者が記述した**韓国語文字列**が翻訳されるかどうかを検証するためのフィクスチャーです。`$[ product_name ]$` ドキュメントのタグは、実際のユーザーガイド (Private-DNS、Storage-Object-Storage、Storage-Online-NAS、nhn-cloud-foundry) からそのまま取得した形式です。
...
以下の表の行はマクロが生成します。第二引数は説明の前に付く接頭辞です。
...
| 項目 | 必須 | 説明 |
|---|---|---|
| ユーザープロンプトテンプレート | X | 行ごとの入力値構成テンプレートです。{{ カラム名 }} パターンが対応するカラム値に置換され、設定すると結合区切り文字より優先して適用されます。カラム名は大文字と小文字を区別します。 |
...
共用環境では、コンソールから直接設定できます。この段落は開きタグが直上に付いているため、増分翻訳ではタグと一つのユニットになります。設定は保存後すぐに適用されます。
{% endif %}
...
コードブロック内のタグは例示テキストであるため、1文字も変更してはなりません。
