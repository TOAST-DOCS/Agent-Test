...
!!! note "参考"
    ...
    オブジェクトロックコンテナは、アーカイブコンテナ$[ "またはレプリケーション対象コンテナ" if replication else "" ]$として指定することはできません。この制限は解除することはできません。
...
| 項目 | 必須 | 説明 |
|---|---|---|
| ユーザープロンプトテンプレート | X | 行ごとの入力値構成テンプレート。{{ カラム名 }} パターンが該当カラムの値に置換されます。設定すると、結合区切り文字より優先して適用されます。カラム名は大文字と小文字を区別します。 |
...
共有環境では、コンソールから直接設定できます。この段落は開始タグが直上に付いているため、増分翻訳においてタグと同一ユニットになります。設定は保存後すぐに反映されます。
{% endif %}
