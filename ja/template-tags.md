...
!!! note "参考"
    ...
    オブジェクトロックコンテナはアーカイブコンテナ$[ " またはレプリケーション対象コンテナ" if replication else "" ]$に指定することはできません。この制限は解除することはできません。
...
| 項目 | 必須 | 説明 |
|---|---|---|
| ユーザープロンプトテンプレート | X | 行ごとの入力値を構成するテンプレートです。{{ column name }} パターンが該当するカラム値に置換され、設定した場合は結合区切り文字より優先して適用されます。カラム名は大文字と小文字を区別します。 |
...
共有環境では、コンソールから直接設定できます。この段落は開始タグが直上に隣接しているため、増分翻訳においてタグと1つのユニットになります。設定は保存と同時に適用されます。
{% endif %}
