...
{%- else -%}
  ...
  {%- set region_names  = "韓国（判橋）リージョン<br>韓国（平村）リージョン<br>韓国（光州）リージョン<br>韓国（釜山）リージョン" -%}
  {%- set encrypt       = true -%}
{%- endif -%}
...
| 項目 | 必須 | 説明 |
|---|---|---|
| ユーザープロンプトテンプレート | X | 行ごとの入力値を構成するテンプレートです。`{{ カラム名 }}` パターンが対応するカラムの値に置換されます。設定すると、結合区切り文字より優先して適用されます。カラム名は大文字と小文字を区別します。 |
...
共有環境では、コンソールから直接設定できます。この段落は開始タグがすぐ上に付いているため、増分翻訳ではタグと一つのユニットになります。設定は保存後すぐに適用されます。
{% endif %}
