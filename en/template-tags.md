...
|---|---|---|
| User prompt template | X | Template for configuring per-row input values. The {{ column name }} pattern is replaced with the corresponding column value. When set, this takes precedence over the combination separator. Column names are case-sensitive. |
...
In a shared environment, you can configure it directly from the console. This paragraph has the opening tag attached directly above it, so it becomes a single unit with the tag in incremental translation. The settings take effect immediately upon saving.
{% endif %}
