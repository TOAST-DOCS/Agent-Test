...
!!! note "Note"
    ...
    An object lock container cannot be designated as an archive container$[ " or a replication target container" if replication else "" ]$. This restriction cannot be lifted.
...
|---|---|---|
| User prompt template | X | Template for configuring input values per row. The {{ column name }} pattern is replaced with the corresponding column value. When set, this takes priority over the combination delimiter. Column names are case-sensitive. |
...
In a shared environment, you can configure this directly in the console. This paragraph has the opening tag immediately above it, making it a single unit with the tag in incremental translation. The settings are applied immediately after saving.
{% endif %}
