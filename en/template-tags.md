...
|---|---|---|
| User prompt template | X | A template for configuring input values per row. The `{{ column name }}` pattern is replaced with the corresponding column value. When set, this takes priority over the join delimiter. Column names are case-sensitive. |
...
In a public environment, you can configure the settings directly in the console. This paragraph is attached immediately below the opening tag, making it a single unit with the tag in incremental translation. Settings are applied as soon as they are saved.
{% endif %}
