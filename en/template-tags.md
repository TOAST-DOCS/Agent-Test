...
|---|---|---|
| User prompt template | X | Template for configuring input values per row. The {{ column name }} pattern is replaced with the corresponding column value. When set, this takes precedence over the combination separator. Column names are case-sensitive. |
...
In a public environment, you can configure settings directly from the console. This paragraph has the opening tag immediately above it, so it becomes one unit with the tag in incremental translation. Settings are applied as soon as they are saved.
{% endif %}
