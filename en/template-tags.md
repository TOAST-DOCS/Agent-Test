...
!!! note "Note"
    ...
    An object lock container cannot be designated$[ " as an archive container or a replication target container" if replication else " as an archive container" ]$. This restriction cannot be lifted.
...
|---|---|---|
| User prompt template | X | Template for configuring input values per row. The {{ column name }} pattern is replaced with the corresponding column value. When set, this takes precedence over the combination delimiter. Column names are case-sensitive. |
...
In a shared environment, you can configure the settings directly from the console. This paragraph is attached directly below the opening tag, making it one unit with the tag in incremental translation. The settings are applied immediately upon saving.
{% endif %}
