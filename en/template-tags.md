...
|---|---|---|
| User prompt template | X | Template for configuring input values per row. The {{ column name }} pattern is replaced with the corresponding column value. When set, this takes precedence over the join separator. Column names are case-sensitive. |
...
In a shared environment, you can configure it directly in the console. This paragraph has the opening tag directly above it, so it becomes a single unit with the tag in incremental translation. The settings are applied immediately upon saving.
{% endif %}
