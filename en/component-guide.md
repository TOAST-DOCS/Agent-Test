...
| log_timestamps | Default log time of MySQL 5.7 is displayed in `UTC` time format; therefore, change log time to `SYSTEM` local time. | Partially supported |
...
| sysdate-is-now | For sysdate, SQL with `sysdate()` used for replication results in discrepant time between Master and Slave, so `sysdate()` and `now()` functions will behave the same. | Partially supported |
