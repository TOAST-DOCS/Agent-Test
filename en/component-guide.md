...
| innodb\_log\_file\_size | Specifies the size of the log file that stores the `redo log` for a transaction. <br><br>Recommended size is 256MB or later in actual environment, and it is set as 512MB by default. In order for the changes to take effect, please restart the database. | O |
