...
The following table does not exist in the same section in en/ja. Since the number of tables in this section differs from ko, the maintenance run must rebuild this section's body based on ko.
...
The corresponding sections in en/ja have the same number of rows and columns as ko's, but the identifiers in the first column are overwritten with format names. Since the ko identifier row is missing from the paired table, it must be recreated.
...
The en/ja sections are missing one identifier row from the table. The remaining rows are correct.
