...
You can access the instance through the serial console in situations where you cannot use the SSH client, such as boot failures and network configuration issues.

The serial console has the following limitations:

* Only one serial console connection per instance is permitted. Multiple connection attempts may not connect properly.
* Serial console access is not guaranteed for instances created from user-uploaded or private images.
* Serial console connections are available for up to 10 minutes.
* Windows instances do not support the serial console feature.
* For instances created before the January 27, 2026 deployment, you must use **Stop Instance** and then **Start Instance**. The **Reboot Instance** function does not apply.

> [Caution]
> Changing the boot method by accessing the instance through the serial console may cause boot failure, and the user is responsible for the consequences.
...
Apply the modified configuration. The command to apply GRUB configuration may differ depending on the OS.
...
## Added section for concurrent PR testing { #concurrent-b-added }

This section was added by PR B to verify concurrent PR scenarios. PR A's translation must not remove this section's translation.
