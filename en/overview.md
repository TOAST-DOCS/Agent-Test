...
An instance is a virtual server composed of virtual CPUs, memory, and root block storage. You can install your services and applications on this server and use it in combination with the various services provided by NHN Cloud. You can create an instance within a few minutes, and you can scale it up or down at any time as needed.

<a id="components"></a>
## Instance components overview { #components }

An instance is composed of the following components:
...
| Flavor | Description |
| ------- |--------------------------------------------------------------------------------------------------------------------------------------------------|
| m2 | A flavor with a balanced CPU and memory configuration. Use this when the performance requirements of your service or application are not clearly defined. |
| c2 | A flavor with high CPU performance. Use this for web application servers or analytics systems that require high-performance computing. |
| r2 | Use this when memory usage is high relative to other resources. Typically used for in-memory databases or cache servers. |
| t2 | A low-cost instance flavor. Use this for servers with low workloads. |
| u2 | The lowest-cost instance flavor. Use this for servers with low workloads.<br>Because it uses local block storage, it is relatively less stable than other instances, but is available at a lower price.<br>This flavor does not guarantee I/O performance. |
...
> [Note]
> Key pairs are resources assigned to user accounts, so they are not deleted when a project is deleted and are retained.
...
* You are charged from the moment an instance is created.
* Root block storage for an instance is charged separately from the instance, based on the block storage pricing.
* When an instance is stopped, a 90% discount from the standard price listed on the website is applied for 90 days. If the instance remains stopped for more than 90 days, the standard price applies while the instance remains in the stopped state.
...
Instances are available in multiple NHN Cloud regions, and each region consists of independent availability zones. When selecting a region, consider the location of your target users, regulatory requirements, and compatibility with other NHN Cloud services.

- **Korea (Pangyo) region**: Provides the lowest latency for domestic services.
- **Korea (Pyeongchon) region**: Well suited for use as a disaster recovery site for the Pangyo region.
- **Japan region**: Suitable for services targeting users in Japan and East Asia.

The instance flavors and images available in each region may differ, so check the list of supported options by region in the console before creating an instance.
...
#### How to access Linux instances from Windows using the PuTTY SSH client

The PuTTY SSH client is a widely used SSH client program on Windows. Install [PuTTY](https://www.chiark.greenend.org.uk/~sgtatham/putty/latest.html) or [iPuTTY](https://github.com/iPuTTY/iPuTTY/releases/tag/l0.70i), which includes a Korean language patch.
...
PuTTY requires that you convert the key pair's private key into PuTTY's private key format before use. Use puttygen, which is installed along with PuTTY, to convert the key.
...
Under **Actions**, click **Save private key** next to **Save the generated key** to save the converted PuTTY-compatible private key. If you leave the **Key passphrase** field blank and save the private key, a message appears asking **Are you sure you want to save this key without a passphrase to protect it?**. To store the converted private key more securely, set a passphrase before saving.
...
* Register the private key file in PuTTY for authentication
* Register the private key file in pageant (PuTTY authentication agent) for authentication

**A. Register the private key file in PuTTY for authentication**
...
**B. Register the private key file in pageant (PuTTY authentication agent) for authentication**
...
To verify that the private key has been added, select **View Keys**. If the key was added successfully, the added key is displayed as shown below.
...
Once the PuTTY-compatible private key is properly registered, run PuTTY.

![Image 6](http://static.toastoven.net/prod_instance/putty009.png)

For the **Host Name** in the basic access information, use the following formats:
...
Set the **Port** to 22, which is the default SSH port, and the **Connection type** to **SSH**.
...
The private key of the key pair that you enter in **Confirm Password** is not transmitted to the server. It is used only in the browser to decrypt the password.

Click the **Connect** button next to **Confirm Password** to download and run the .rdp file, which contains the remote desktop connection settings, and then connect to the Windows server. The Windows server ID is `Administrator`, and use the password that you confirmed in the NHN Cloud console.

<a id="how-to-connect-serial-console"></a>
### How to connect to the serial console { #how-to-connect-serial-console }

You can connect to an instance through the serial console when an SSH client cannot be used, such as in cases of boot failure or network configuration issues.

The serial console feature has the following constraints:

* Only one serial console connection is allowed per instance. Multiple connection attempts may result in a failed connection.
* Serial console access is not guaranteed for instances created from user-uploaded images or personal images.
...
* Instances created before the January 27, 2026 deployment require an **Instance Stop** followed by an **Instance Start**. The **Reboot Instance** feature does not apply this change.

> [Caution]
> Changing the boot method while connected to an instance through the serial console may cause a boot failure, and the user is responsible for the results.
> In normal situations, we recommend that you use an SSH client connection.

<a id="change-grub-bootloader-settings"></a>
#### Change GRUB bootloader settings

To manipulate the bootloader on instances created before the November 26, 2024 deployment, GRUB configuration is required.
...
Apply the updated configuration. The command to apply GRUB settings may differ depending on the operating system.
