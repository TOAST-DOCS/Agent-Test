...
- **Tags**: User-defined labels for categorizing and searching instances
...
This section is displayed only on sites built with the `this-is-only-public` flag. It is not shown in documentation for other zones or environments.

* Example: Policies, pricing, and SLA notices that apply only to public regions
* Example: Promotional text to display only for commercial deployment targets
<!-- @endif -->
...
To protect the data in your instance, we recommend that you regularly create backups and snapshots. Backups and snapshots differ as follows:

- **Backup**: Periodically copies the instance's root block storage according to a specified schedule and stores it in a separate storage. It is suitable for long-term retention and disaster recovery.
- **Snapshot**: Saves the state of the instance at a specific point in time. It is suitable for use as a rollback point before system changes.

Backups and snapshots are retained even if the original instance is deleted, so you can use them to restore the instance after deletion. For more information, see [Backup Service Overview](/Storage/Backup/en/overview/).
