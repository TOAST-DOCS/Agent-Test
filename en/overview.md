...
#### How to access a Linux instance from an SSH client on Mac or Linux
...
You can connect to the serial console to access your instance in situations where you cannot use an SSH client, such as boot failures or network configuration issues.
...
* Instances created with user-uploaded images or personal images do not guarantee serial console access.
...
* For instances created before January 27, 2026, you must **stop the instance** and then **start the instance** after the deployment. The **reboot instance** feature does not apply the changes.

> [Caution]
> If you change the boot method by accessing your instance through the serial console, the instance may fail to boot, and the user is responsible for the consequences.
> We recommend that you use SSH client connection in normal situations.

#### GRUB Bootloader Configuration Changes

For instances created before the deployment on November 26, 2024, GRUB configuration is required to manipulate the bootloader.
...
Apply the changed configuration. The GRUB configuration command may differ depending on the OS.
...
This section is added by PR B to verify concurrent PR scenarios. PR A's translation must not delete the translation of this section.
