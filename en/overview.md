...
### How to Connect to the Serial Console

When you cannot use an SSH client, such as in cases of boot failure or network configuration issues, you can connect to the serial console to access the instance.

The serial console feature has the following limitations:

* Only one serial console connection per instance is allowed, and attempting multiple connections may not connect properly.
* Serial console access is not guaranteed for instances created from user-uploaded or personal images.
* You can access the serial console for a maximum of 10 minutes.
* Windows instances do not support the serial console feature.
* For instances created before January 27, 2026 deployment, **stopping the instance** and then **starting the instance** is required. The **restart instance** feature does not apply.

> [Caution]
> When you access the instance through the serial console and change the boot method, booting may fail, and you are responsible for the consequences.
> In normal situations, we recommend that you use an SSH client.

#### Change GRUB bootloader settings

To manipulate the bootloader on instances created before the November 26, 2024 release, you need GRUB configuration.
...
Apply the modified configuration. The GRUB configuration command may differ depending on the OS.
...
This section was added by PR B to verify concurrent PR scenarios. PR A's translation must not remove the translation of this section.
