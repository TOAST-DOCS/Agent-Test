...
You can connect to the serial console to access your instance in situations where you cannot use an SSH client, such as boot failure or network configuration issues.

The serial console feature has the following limitations:

* Only one serial console connection per instance is possible. Multiple connection attempts may result in connection failure.
* Instances created from user-uploaded images or personal images do not guarantee serial console access.
* Serial console connections are available for a maximum of 10 minutes.
* Windows instances do not support the serial console feature.
* For instances created before the January 27, 2026 deployment, you must **Stop the instance** and then **Start the instance**. The **Reboot instance** function does not apply this change.

> [Caution]
> If you change the boot method by connecting to your instance through the serial console, the boot may fail, and the user is responsible for the consequences.
> In normal situations, we recommend that you use SSH client connection.
...
## Concurrent PR test addition section { #concurrent-b-added }
