...
* Only one serial console connection per instance is allowed. Multiple connection attempts may result in connection failures.
* Instances created from personally uploaded images or personal images do not guarantee serial console access.
...
* For instances created before January 27, 2026 deployment, you must **Stop instance** and then **Start instance**. The **Reboot instance** feature will not apply the changes.

> [Caution]
> Changing the boot method by accessing your instance through the serial console may cause it to fail to boot, and the user is responsible for the consequences.
