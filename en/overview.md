...
* Example: Policy, pricing, and SLA information that applies only to public regions
* Example: Promotional content to display only for commercial deployment targets
<!-- @endif -->
...
We recommend that you perform regular backups and create snapshots to protect your instance data safely. Backups and snapshots differ as follows.

- **Backup**: Periodically copies the instance's root block storage according to a specified schedule and stores it in a separate repository. Suitable for long-term retention and disaster recovery.
...
Backups and snapshots are retained even after the original instance is deleted, so you can use them to restore an instance after deletion. For more details, see [Backup Service Overview](/Storage/Backup/en/overview/).
