...
The Instance API uses the `compute` type endpoint. The exact endpoint is referenced in the `serviceCatalog` of the token issuance response.
...
API responses may display fields not specified in this guide. These fields are for NHN Cloud internal use and may change without prior notice, so do not use them.
...
| minDisk | Query | Integer | - | Minimum Block Storage size in GB<br>Returns only types with Block Storage size larger than the specified size |
| minRam | Query | Integer | - | Minimum RAM size in MB<br>Returns only types with RAM size larger than the specified size |
...
| flavors.links | Body | Object | Instance type path object |
| flavors.name | Body | String | Instance type name |
| flavors.ram | Body | Integer | Memory size in MB |
...
| flavors.swap | Body | Integer | Swap area size in GB |
...
| flavors.OS-FLV-EXT-DATA:ephemeral | Body | Integer | Ephemeral block storage size in GB |
| flavors.disk | Body | Integer | Root block storage size in GB |
...
| `HARD_REBOOT` | The instance is undergoing a hard reboot.<br> This is equivalent to powering off and then powering on the physical server. |
| `MIGRATING` | The instance is migrating.<br> This occurs due to live migration (active instance movement) operations. |
...
| `REBOOT` | The instance is undergoing a soft reboot.<br> The reboot command is sent to the virtual machine operating system. |
| `REBUILD` | The instance is being rebuilt from its original image. |
| `RESCUE` | The instance is running in rescue mode. |
| `RESIZE` | The instance type is being changed or the instance is being migrated to another host.<br> The instance is stopped and then restarted. |
| `REVERT_RESIZE` | The instance type change or migration to another host has failed, and the instance is being reverted to its original state. |
| `VERIFY_RESIZE` | The instance has completed the type change or migration to another host and is awaiting user approval.<br> In NHN Cloud, the instance automatically becomes `ACTIVE` in this case. |
...
| `UNKNOWN` | The instance status is unknown.<br> `If the instance enters this state, contact the administrator.` |
...
| tenantId | URL | String | O | Tenant ID |
| tokenId | Header | String | O | Token ID |
| reservation_id | Query | String | - | Instance creation reservation ID. <br>When specified, returns only instances created simultaneously. |
| changes-since | Query | Datetime | - | Return the list of instances that were changed after the specified time. Format: `YYYY-MM-DDThh:mm:ss` |
| image | Query | UUID | - | Image ID<br>Return the list of instances using the specified image. |
| flavor | Query | UUID | - | Instance type ID<br>Return the list of instances using the specified type. |
| name | Query | String | - | Instance name<br>Return the list of instances with the specified name; can be queried using regular expressions. |
| status | Query | Enum | - | Instance status<br>Return the list of instances with the specified status. |
| limit | Query | Integer | - | Number of instances in the list<br>Return the list of instances up to the specified number. |
| marker | Query | UUID | - | The first instance UUID in the list<br>Return the list of instances from the instance specified by `marker` up to the number specified by `limit` according to the sort criteria. |
...
| Name | Type | Format | Description |
|---|---|---|---|
| servers | body | Object | Server list object |
...
| servers.hostId | Body | String | Host ID where the instance is running |
| servers.addresses | Body | Object | Server IP list object. <br>A list is created for each port connected to the instance. |
...
| servers.OS-EXT-STS:task_state | Body | String | Instance task state<br>Indicates the progress of an operation when an action is performed on the instance |
| servers.OS-EXT-STS:vm_state | Body | String | Current state of the instance |
| servers.OS-SRV-USG:launched_at | Body | Datetime | Instance last boot time<br>`YYYY-MM-DDThh:mm:ss.ssssss` format |
...
| servers.security_groups.name | Body | String | Security group name assigned to the instance |
| servers.user_id | Body | String | User ID who created the instance |
| servers.created | Body | Datetime | Instance creation time in `YYYY-MM-DDThh:mm:ssZ` format |
| servers.tenant_id | Body | String | Tenant ID where the instance belongs |
| servers.os-extended-volumes:volumes_attached | Body | Object | Additional block storage list object connected to the instance |
| servers.os-extended-volumes:volumes_attached.id | Body | UUID | Additional block storage ID connected to the instance |
...
| server.NHN-EXT-ATTR:ephemeral_disk_size | Body | Integer | Additional local block storage size connected to the instance |
...
| server.addresses | Body | Object | List of instance IP addresses<br>A list is created for each port connected to the instance |
...
| server.addresses."Network name".version | Body | Integer | IP version of the port connected to the instance<br>NHN Cloud supports IPv4 only |
...
| server.OS-EXT-STS:task_state | Body | String | Instance task state<br>Shows the progress of the operation when an action is performed on the instance |
| server.OS-EXT-STS:vm_state | Body | String | Current instance state |
...
| server.tenant_id | Body | String | ID of the tenant to which the instance belongs |
| server.os-extended-volumes:volumes_attached | Body | Object | List of additional Block Storage connected to the instance |
| server.os-extended-volumes:volumes_attached.id | Body | UUID | ID of additional Block Storage connected to the instance |
