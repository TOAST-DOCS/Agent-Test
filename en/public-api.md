...
| `MIGRATING` | When the instance is migrating<br/>This occurs during a real-time migration (moving active instances) |
...
| `REBOOT` | Instance is in a soft reboot state<br/> The reboot command is passed to the virtual machine operating system |
...
| `UNKNOWN` | Instance status is unknown<br/>If the instance enters this state, contact an administrator. |
...
| server.user_data | Body | String | - | Script to be executed after instance booting and its configuration<br/>Base64-encoded string up to 65535 bytes |
| server.availability_zone | Body | String | - | Availability zone for instance creation<br/>If not specified, randomly selected<br/>If the source type of the root block storage is `volume`, `snapshot`, it must be set to the same availability zone as the source block storage |
