...
Instance API uses the `compute` type endpoint. For the exact endpoint, refer to `serviceCatalog` in the token issuance response.
...
| minDisk | Query | Integer | - | Minimum block storage size in GB<br>Returns only instance types with block storage size larger than the specified size |
| minRam | Query | Integer | - | Minimum RAM size in MB<br>Returns only instance types with RAM size larger than the specified size |
...
| flavors.links | Body | Object | Instance type resource links |
...
### List Instance Type Details { #list-flavors-with-details }
...
| flavors.swap | Body | Integer | Swap space size in GB  |
| flavors.os-flavor-access:is_public | Body | Boolean | Public flag          |
...
| `ERROR` | The previous operation failed |
| `HARD_REBOOT` | Instance is force restarted<br>Equivalent to powering off and powering on a physical server |
| `MIGRATING` | Instance is migrating<br>Occurs due to live migration operations that move an active instance |
| `PASSWORD` | Password reset is in progress |
...
| `REBUILD` | Instance is being recreated from the original image |
| `RESCUE` | Instance is running in recovery mode |
| `RESIZE` | Instance type is being changed or the instance is being moved to another host<br>The instance is stopped and then restarted |
| `REVERT_RESIZE` | Instance is being recovered to the original state after a failed resize or host migration |
| `VERIFY_RESIZE` | The instance has completed resize or host migration and is waiting for user approval<br>In NHN Cloud, the instance automatically transitions to the `ACTIVE` status in this case |
| `SHELVED_OFFLOADED` | Instance is shelved offloaded |
| `SHUTOFF` | Instance is shut off |
| `SUSPENDED` | Instance is suspended by the administrator |
| `UNKNOWN` | Instance status is unknown<br>`Contact the administrator if the instance enters this state.` |
...
| changes-since | Query | Datetime | - | Return a list of instances modified after the specified time. In the format `YYYY-MM-DDThh:mm:ss`. |
| image | Query | UUID | - | Image ID<br>Return a list of instances using the specified image. |
| flavor | Query | UUID | - | Instance type ID<br>Return a list of instances using the specified type. |
| name | Query | String | - | Instance name<br>Return a list of instances with the specified name. Queryable using regular expressions. |
| status | Query | Enum | - | Instance status<br>Return a list of instances with the specified status. |
| limit | Query | Integer | - | Number of instances in the list<br>Return a list of instances up to the specified number. |
| marker | Query | UUID | - | First instance UUID in the list<br>Based on the sort criteria, return the list of instances starting from the instance specified by `marker`, up to the number specified by `limit`. |
...
| servers.addresses."Network name" | Body | Object | Port information per network connected to the instance |
| servers.addresses."Network name".OS-EXT-IPS-MAC:mac_addr | Body | String | MAC address of the port connected to the instance |
| servers.addresses."Network name".version | Body | Integer | IP version of the port connected to the instance<br>NHN Cloud only supports IPv4 |
...
| servers.os-extended-volumes:volumes_attached | Body | Object | List object of additional block storage connected to the instance |
...
| server.addresses."Network Name".OS-EXT-IPS:type | Body | Enum | Type of IP address for the port<br>Either `fixed` or `floating` |
...
| server.created | Body | Datetime | Time when the instance was created in `YYYY-MM-DDThh:mm:ssZ` format |
| server.tenant_id | Body | String | ID of the tenant to which the instance belongs |
| server.os-extended-volumes:volumes_attached | Body | Object | List object of additional block storage connected to the instance |
