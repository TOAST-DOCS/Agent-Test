...
| `MIGRATING` | When an instance is being migrated<br/> This is caused by a real-time migration (moving active instances) |
...
| `REBOOT` | Instance is in a soft reboot state<br/> The reboot command is sent to the virtual machine operating system. |
...
| `UNKNOWN` | Instance status is unknown<br/>If an instance enters this state, contact your administrator. |
...
| limit | Query | Integer | - | Number of instances<br/>Return list with up to specified number of instances |
...
| servers.addresses | Body | Object | Instance IP list object. <br/>A list is created for each port attached to the instance. |
...
| server.security_groups | Body | Object | - | Security group list object<br/>If left blank, the `default` group is added. |
| server.security_groups.name | body | String | - | **(Conditionally required)** Name of the security group to add to the instance |
| server.user_data | Body | String | - | Script to be executed after instance booting and its configuration<br/>Base64-encoded string up to 65,535 bytes |
| server.availability_zone | Body | String | - | Availability zone in which to create the instance<br/>If not specified, a random availability zone is selected<br/>If the source type of the root block storage is `volume`, `snapshot`, it must be set to the same availability zone as the source block storage |
...
| server.block_device_mapping_v2.boot_index | Body | Integer | O | Boot order of the specified block storage<br/>- If `0`, root block storage<br/>- If not, additional block storage<br/>The larger the boot index, the lower the boot priority |
...
| server.min_count | Body | Integer | - | The minimum number of instances to create with the current request.<br/>Default value is 1<br/>Can only be set to `1` if the source type of the block storage is `volume` |
| server.max_count | Body | Integer | - | Maximum number of instances to create with the current request.<br/>Default is min_count, maximum is 10<br/>Can only be set to `1` if the source type of the block storage is `volume` |
