...
| minDisk | Query | Integer | - | Minimum block storage size (GB)<br>Returns only flavors with block storage size larger than the specified size |
| minRam | Query | Integer | - | Minimum RAM size (MB)<br>Returns only flavors with RAM size larger than the specified size |
...
|---|---|---|---|
| flavors | Body | Object | Instance flavors list object |
...
| `HARD_REBOOT` | The instance is being forcefully restarted<br>This is equivalent to shutting down and turning on the power on a physical server |
...
| `REBOOT` | The instance is in a soft reboot state<br>The reboot command is delivered to the virtual machine's operating system |
| `REBUILD` | The instance is being recreated from the image at the time of creation |
...
| `REVERT_RESIZE` | The instance is being restored to its original state due to a failure during the process of changing the instance type or moving the instance to another host |
| `VERIFY_RESIZE` | The instance has completed the process of changing its type or being moved to another host and is waiting for user confirmation<br>NHN Cloud automatically transitions the instance to `ACTIVE` status in this case |
...
| limit | Query | Integer | - | Number of instances in the list<br>Returns the number of instances specified |
| marker | Query | UUID | - | UUID of the first instance in the list<br>Returns a list of instances starting from the instance specified as `marker` and limited to the `limit` count, based on the sorting criteria |
...
Returns a list of instances created in the current tenant, the same as the List Instances operation. However, detailed information for each instance is also retrieved.
...
| servers.OS-EXT-STS:task_state | Body | String | Instance task state<br>Shows the progress status of an operation when an action is performed on the instance |
| servers.OS-EXT-STS:vm_state | Body | String | Instance current state |
...
| server.OS-EXT-STS:task_state | Body | String | Instance task state<br>Shows the progress of an action when an operation is performed on the instance                                                                                                                                                               |
...
| server.security_groups.name | Body | String | Security group name assigned to the instance                                                                                                                                                                                       |
...
| server.os-extended-volumes:volumes_attached.id | Body | UUID | Additional block storage ID connected to the instance                                                                                                                                                                                  |
