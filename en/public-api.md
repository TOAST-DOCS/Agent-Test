...
| Name | Type | Format | Description |
|---|---|---|---|
| keypairs | Body | Array | Key pair list |
...
| Name | Type | Format | Description |
|---|---|---|---|
| keypair | Body | Object | Key pair list |
...
| Status | Description |
|---|---|
| `ACTIVE` | Instance is in the active state. |
| `BUILD` | Instance is being created. |
| `DELETED` | Instance has been deleted. |
| `ERROR` | The previous operation on the instance failed. |
| `HARD_REBOOT` | Instance has been forcibly rebooted.<br>This is equivalent to powering off and restarting a physical server. |
| `MIGRATING` | Instance is being migrated.<br>This occurs due to live migration (active instance movement) operations. |
...
| `REBOOT` | Instance is in soft reboot state.<br>The reboot command is delivered to the virtual machine's operating system. |
| `REBUILD` | Instance is being recreated from the image used at creation time. |
| `RESCUE` | Instance is running in rescue mode. |
| `RESIZE` | Instance type is being changed or the instance is being moved to another host.<br>The instance has been stopped and then restarted. |
| `REVERT_RESIZE` | Instance is being restored to its original state when the process of changing the instance type or moving to another host fails. |
| `VERIFY_RESIZE` | Instance has completed the process of type change or being moved to another host and is waiting for user approval.<br>In NHN Cloud, the instance automatically becomes `ACTIVE` in this case. |
| `SHELVED_OFFLOADED` | Instance has been shut down. |
| `SHUTOFF` | Instance has been stopped. |
| `SUSPENDED` | Instance has entered hibernation mode by the administrator. |
...
| name | Query | String | - | Instance name<br>Returns the list of instances with the specified name. Queryable using regular expressions. |
| status | Query | Enum | - | Instance status<br>Returns the list of instances with the specified status. |
| limit | Query | Integer | - | Number of instances in the list<br>Returns the specified number of instances in the list. |
| marker | Query | UUID | - | First instance UUID in the list<br>Based on the sort criteria, returns the specified number of instances starting from the instance designated by `marker`. |
