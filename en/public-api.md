...
| `MIGRATING` | When an instance is migrating<br/> This is caused by a real-time migration (moving active instances) |
...
| `REBOOT` | Instance is in a soft reboot state<br/>The reboot command is transmitted to the virtual machine operating system |
...
| server.user_data | Body | String | - | Script to be executed after instance booting and its configuration<br/>Base64-encoded string; up to 65,535 bytes allowed |
| server.availability_zone | Body | String | - | The availability zone in which to create the instance.<br/>If not specified, a zone is selected randomly.<br/>If the source type of the root block storage is `volume` or `snapshot`, it must be set to the same availability zone as the source block storage. |
...
| server.return_reservation_id | Body | Boolean | - | Reservation ID for the instance creation request.<br/>If set to `True`, reservation ID is returned instead of instance creation information.<br/>Default is `False` |
