...
<a id="list-flavors"></a>
### List Instance Flavors { #list-flavors }
...
| minDisk | Query | Integer | - | Minimum Block Storage size (GB)<br>Returns instance types with Block Storage size greater than the specified size |
| minRam | Query | Integer | - | Minimum RAM size (MB)<br>Returns instance types with RAM size greater than the specified size |
...
|---|---|---|---|
| flavors | Body | Object | Flavor list object |
| flavors.id | Body | UUID | Flavor ID |
| flavors.links | Body | Object | Flavor path object |
...
### List Instance Flavors with Details { #list-flavors-with-details }
...
| flavors.OS-FLV-DISABLED:disabled | Body | Boolean | Enabled         |
...
| flavors.os-flavor-access:is_public | Body | Boolean | Public          |
...
### View Availability Zones { #list-availability-zones }
...
| `HARD_REBOOT` | The instance is in a forced reboot state<br> This is equivalent to power-cycling the physical server |
| `MIGRATING` | The instance is being migrated<br> This occurs due to live migration (moving active instances) |
...
| `REBOOT` | The instance is in a soft reboot state<br> A reboot command is sent to the virtual machine's operating system |
| `REBUILD` | The instance is being rebuilt from the original image |
| `RESCUE` | The instance is running in recovery mode |
| `RESIZE` | The instance type is being changed or the instance is being moved to another host<br>The instance is stopped and then restarted |
| `REVERT_RESIZE` | The instance is being recovered to its original state after a failed resize or migration operation |
| `VERIFY_RESIZE` | The instance has completed a resize or migration operation and is waiting for user confirmation<br>In NHN Cloud, the instance automatically becomes `ACTIVE` |
| `SHELVED_OFFLOADED` | The instance has been shut down |
| `SHUTOFF` | The instance is stopped |
| `SUSPENDED` | The instance has been put into hibernation mode by an administrator |
| `UNKNOWN` | The instance status is unknown<br>`If the instance enters this state, contact your administrator.` |
...
| changes-since | Query | Datetime | - | Instances modified since the specified time. Format: `YYYY-MM-DDThh:mm:ss`. |
| image | Query | UUID | - | Image ID<br>Instances using the specified image. |
| flavor | Query | UUID | - | Instance type ID<br>Instances using the specified type. |
| name | Query | String | - | Instance name<br>Instances with the specified name. Queryable using regular expressions. |
| status | Query | Enum | - | Instance status<br>Instances with the specified status. |
| limit | Query | Integer | - | Number of instances<br>Returns the specified number of instances. |
| marker | Query | UUID | - | First instance UUID in the list<br>Returns instances starting from the one specified by `marker` up to the number specified by `limit`, based on sort criteria. |
...
Request format is the same as List Instances.
...
| servers.addresses | Body | Object | Instance IP list object. <br>Lists are created for each port connected to the instance. |
...
| servers.addresses."Network name".OS-EXT-IPS:type | Body | Enum | Port IP address type<br>Either `fixed` or `floating` |
...
| servers.OS-EXT-STS:task_state | Body | String | Instance task state<br>Shows the task progress when an action is performed on the instance |
...
| servers.os-extended-volumes:volumes_attached | Body | Object | Additional block storage list object attached to the instance |
| servers.os-extended-volumes:volumes_attached.id | Body | UUID | Additional block storage ID attached to the instance |
| servers.OS-EXT-STS:power_state | Body | Integer | Power state of the instance<br>- `1`: On<br>- `4`: Off |
| servers.metadata | Body | Object | Instance metadata object<br>Instance metadata stored as key-value pairs |
| server.NHN-EXT-ATTR:ephemeral_disk_size | Body | Integer | Additional local block storage size attached to the instance |
| server.NHN-EXT-ATTR:protect | Body | Boolean | Instance deletion protection |
...
| server.hostId | Body | String | ID of the host where the instance is running |
...
| server.addresses."Network 이름" | Body | Object | Port information for the network connected to the instance |
| server.addresses."Network 이름".OS-EXT-IPS-MAC:mac_addr | Body | String | MAC address of the port connected to the instance |
| server.addresses."Network 이름".version | Body | Integer | IP version of the port connected to the instance<br>NHN Cloud supports only IPv4 |
| server.addresses."Network 이름".addr | Body | String | IP address of the port connected to the instance |
| server.addresses."Network 이름".OS-EXT-IPS:type | Body | Enum | IP address type of the port<br>`fixed` or `floating` |
...
| server.OS-EXT-STS:task_state | Body | String | Instance task state<br>Indicates task progress status when an action is performed on the instance |
| server.OS-EXT-STS:vm_state | Body | String | Instance current state |
| server.OS-SRV-USG:launched_at | Body | Datetime | Last boot time of instance<br>`YYYY-MM-DDThh:mm:ss.ssssss` format |
...
| server.metadata | Body | Object | Instance metadata object<br>Instance metadata is stored in key-value pairs |
