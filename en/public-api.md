...
The Instance API uses a `compute` type endpoint. Refer to the `serviceCatalog` of the token issuance response for the exact endpoint.
...
|---|---|---|---|
| flavors | Body | Object | Instance types object |
| flavors.id | Body | UUID | Instance type ID |
| flavors.links | Body | Object | Instance type links |
...
### List Flavors in Detail { #list-flavors-with-details }
...
| flavors.os-flavor-access:is_public | Body | Boolean | Public access status |
...
| keypair | Body | Object | Key pair object |
| keypair.public_key | Body | String | Public key |
| keypair.private_key | Body | String | Private key. Returned only if a new key pair is created. |
| keypair.user_id | Body | String | Key pair owner ID |
| keypair.name | Body | String | Key pair name |
| keypair.fingerprint | Body | String | Key pair fingerprint |
...
| `HARD_REBOOT` | Instance is force-rebooting<br>Equivalent to powering off and on the physical server |
| `MIGRATING` | Instance is being migrated<br>This occurs during live migration (moving an active instance) |
| `PASSWORD` | Password is being reset |
| `PAUSED` | Instance is paused<br>Paused instance is stored in hypervisor memory |
| `REBOOT` | Instance is soft-rebooting<br>Reboot command has been sent to the virtual machine's operating system |
| `REBUILD` | Instance is being rebuilt from the original image |
| `RESCUE` | Instance is running in rescue mode |
| `RESIZE` | Instance type is being changed or instance is being moved to another host<br>The instance has been stopped and is restarting |
| `REVERT_RESIZE` | Recovering to the original state due to failure during instance type change or migration |
| `VERIFY_RESIZE` | Instance has completed type change or migration and is awaiting user confirmation<br>In NHN Cloud, the instance automatically transitions to `ACTIVE` status |
...
| Name | Location | Format | Required | Description |
|---|---|---|---|---|
| tenantId | URL | String | O | Tenant ID |
| tokenId | Header | String | O | Token ID |
| reservation_id | Query | String | - | Instance reservation ID. <br>Returns only instances created simultaneously when specified. |
| changes-since | Query | Datetime | - | Returns instances changed after the specified time. Format: `YYYY-MM-DDThh:mm:ss`. |
...
| name | Query | String | - | Instance name<br>Returns instances matching the specified name. Supports regular expressions. |
| status | Query | Enum | - | Instance status<br>Returns instances with the specified status. |
| limit | Query | Integer | - | Maximum number of instances in the list<br>Returns up to the specified number of instances. |
| marker | Query | UUID | - | First instance UUID in the list<br>Based on the sort order, returns up to `limit` instances starting from the instance specified by `marker`. |
...
| servers.OS-EXT-STS:task_state | Body | String | Instance task status<br>Shows the progress status of an action when an action is performed on the instance |
...
| server.OS-EXT-STS:task_state | Body | String | Instance task state<br>Indicates the progress of operation when an action is performed on the instance                                                                                                                                                               |
...
| server.OS-SRV-USG:terminated_at | Body | Datetime | Time when the instance was terminated<br>`YYYY-MM-DDThh:mm:ssZ` format                                                                                                                                                                  |
