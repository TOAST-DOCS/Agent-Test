...
Instance API uses the `compute` type endpoint. For the exact endpoint, refer to the `serviceCatalog` in the token issuance response.
...
The API response may contain fields that are not specified in the guide. These fields are used for internal NHN Cloud purposes and may be changed without prior notice, so do not use them.
...
| minDisk | Query | Integer | - | Minimum Block Storage size (GB)<br>Returns only instance types with Block Storage size greater than the specified size. |
| minRam | Query | Integer | - | Minimum RAM size (MB)<br>Returns only instance types with RAM size greater than the specified size. |
...
| flavors.os-flavor-access:is_public | Body | Boolean | Shareability          |
...
| keypair | Body | Object | Key pair object |
| keypair.public_key | Body | String | Public key |
| keypair.private_key | Body | String | Private key. The private key is returned when a new key pair is created. |
| keypair.user_id | Body | String | Key pair owner ID |
| keypair.name | Body | String | Key pair name |
| keypair.fingerprint | Body | String | Key pair fingerprint |
...
| `HARD_REBOOT` | The instance is being forcefully restarted.<br>This is equivalent to powering off and on a physical server. |
| `MIGRATING` | The instance is being migrated.<br>This occurs due to live migration (moving an active instance) operations. |
...
| `REVERT_RESIZE` | The instance is being restored to its original state after a failed attempt to change the instance type or move it to another host. |
| `VERIFY_RESIZE` | The instance has completed changing its type or being moved to another host and is waiting for user confirmation.<br>In NHN Cloud, the instance automatically transitions to `ACTIVE` status in this case. |
| `SHELVED_OFFLOADED` | The instance has been shelved. |
| `SHUTOFF` | The instance is shut off. |
| `SUSPENDED` | The instance has been suspended by an administrator. |
| `UNKNOWN` | The instance status cannot be determined.<br>`If the instance enters this status, contact the administrator.` |
...
| tenantId | URL | String | O | Tenant ID |
| tokenId | Header | String | O | Token ID |
| reservation_id | Query | String | - | Instance creation reservation ID. <br>When you specify the reservation ID, only the list of instances created simultaneously is returned. |
| changes-since | Query | Datetime | - | Return the list of instances that have changed after the specified time. Format is `YYYY-MM-DDThh:mm:ss`. |
| image | Query | UUID | - | Image ID<br>Return the list of instances that use the specified image. |
| flavor | Query | UUID | - | Instance type ID<br>Return the list of instances that use the specified type. |
| name | Query | String | - | Instance name<br>Return the list of instances with the specified name. Regular expressions can be used for queries. |
| status | Query | Enum | - | Instance status<br>Return the list of instances with the specified status. |
| limit | Query | Integer | - | Number of instances<br>Return the specified number of instances. |
| marker | Query | UUID | - | UUID of the first instance in the list<br>Return the list of instances starting from the instance specified by `marker` for the number specified by `limit`, depending on the sort order. |
...
| servers.addresses | Body | Object | Instance IP list object. <br>A list is created for each port connected to the instance. |
| servers.addresses."Network 이름" | Body | Object | Port information for each network connected to the instance |
| servers.addresses."Network 이름".OS-EXT-IPS-MAC:mac_addr | Body | String | MAC address of the port connected to the instance |
| servers.addresses."Network 이름".version | Body | Integer | IP version of the port connected to the instance<br>NHN Cloud supports only IPv4 |
| servers.addresses."Network 이름".addr | Body | String | IP address of the port connected to the instance |
...
| servers.OS-EXT-STS:task_state | Body | String | Instance task state<br>Indicates the progress of an operation when an operation is performed on the instance |
| servers.OS-EXT-STS:vm_state | Body | String | Current state of the instance |
| servers.OS-SRV-USG:launched_at | Body | Datetime | Last boot time of the instance<br>In `YYYY-MM-DDThh:mm:ss.ssssss` format |
| servers.OS-SRV-USG:terminated_at | Body | Datetime | Deletion time of the instance<br>In `YYYY-MM-DDThh:mm:ssZ` format |
| servers.flavor | Body | Object | Instance type object |
...
| server.addresses | Body | Object | Instance IP address list object <br>The list is created for each port connected to the instance |
...
| server.addresses."Network name".OS-EXT-IPS:type | Body | Enum | Type of IP address of the port<br>One of `fixed` or `floating` |
...
| server.OS-EXT-STS:task_state | Body | String | Instance task state<br>Indicates the progress of the operation when an action is performed on the instance |
...
| server.NHN-EXT-ATTR:ephemeral_disk_size | Body | Integer | Size of additional local block storage attached to the instance |
