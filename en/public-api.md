...
| flavors.OS-FLV-EXT-DATA:ephemeral | Body | Integer | Temporary Block Storage size in GB     |
...
| keypairs.keypair.name | Body | String | Name of the key pair |
...
| keypair.user_id | Body | String | User ID of the key pair owner |
...
| `HARD_REBOOT` | Instance has been forcibly restarted<br>This is equivalent to powering off and on a physical server |
...
| `REBOOT` | Instance is in soft reboot state<br>A reboot command is sent to the virtual machine operating system |
| `REBUILD` | Instance is being rebuilt from the original image |
| `RESCUE` | Instance is running in rescue mode |
| `RESIZE` | Instance type is being changed or instance is being moved to another host<br>Instance has been stopped and restarted |
| `REVERT_RESIZE` | Instance is being reverted to its original state if resizing or migration to another host fails |
| `VERIFY_RESIZE` | Instance is waiting for user approval after type change or migration to another host completes<br>In NHN Cloud, the instance automatically transitions to `ACTIVE` state |
| `SHELVED_OFFLOADED` | Instance has been shelved |
| `SHUTOFF` | Instance has been stopped |
| `SUSPENDED` | Instance has been hibernated by an administrator |
...
| changes-since | Query | Datetime | - | Returns a list of instances changed since the specified time. Format: `YYYY-MM-DDThh:mm:ss`. |
...
| name | Query | String | - | Instance name<br>Returns a list of instances with the specified name; can be queried with regular expressions |
| status | Query | Enum | - | Instance status<br>Returns a list of instances with the specified status |
| limit | Query | Integer | - | Number of instances in the list<br>Returns a list of the specified number of instances |
| marker | Query | UUID | - | UUID of the first instance in the list<br>Based on the sort order, returns a list of instances starting from the instance specified by `marker` for the `limit` number of instances |
