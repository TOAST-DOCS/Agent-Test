...
| server.user_data | Body | String | - | Script to be executed after instance booting and its configuration<br/>String encoded in base64; up to 65,535 bytes allowed |
| server.availability_zone | Body | String | - | Availability zone in which to create the instance<br/>If not specified, it is randomly selected<br/>If the source type of the root block storage is `volume` or `snapshot`, it must be set to the same availability zone as the source block storage |
...
| server.block_device_mapping_v2.boot_index | Body | Integer | O | Boot order of the specified block storage<br/>- If `0`, it is the root block storage<br/>- If not, additional block storage<br/>The larger the size, the lower the boot order |
