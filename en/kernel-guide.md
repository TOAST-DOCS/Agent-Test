...
### Default repository settings { #default-storage-settings }
...
* **name**
  * The name of the repository; you can change it without issue.
* **mirrorlist**
  * Specify a URL that provides a list of mirror servers from which the package can be downloaded.
...
dnf and yum will automatically update the GRUB2 configuration file.
...
Verify the currently configured default kernel.
...
Check the list of currently installed kernels.
...
Select one of the currently installed kernels and change the default kernel.
...
Restart the operating system to apply the boot order changes.
...
Currently, the official Rocky 8.10 image does not allow kernel changes to be made with the grubby command, so I use the shell script below.
...
### Configure default repositories { #rocky-linux-9-default-storage-settings }

Change the default repository to match your system's architecture and Rocky Linux version.
...
* **name**
  * The name of the repository. You can change it without any issue.
* **mirrorlist**
  * Specify a URL that provides a list of mirror servers from which the package can be downloaded.
  * Get a list of mirror servers that match your system's architecture (`$basearch`) and Rocky Linux version (`$releasever`).
* **baseurl**
...
* **gpgcheck**
  * Set the URL or path to the repository that contains the GNU Privacy Guard (GPG) key. The GPG key is a cryptographic signature used to authenticate rpm packages.
* **enabled**
  * This setting enables or disables this repository.
  * A value of 1 enables it, and a value of 0 disables it.
* **countme**
  * This feature collects Rocky Linux usage statistics.
...
* **gpgkey**
  * The path to the GPG key file used to verify package signatures.
...
Delete the cache that stores metadata from previously downloaded packages.
