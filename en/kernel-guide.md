...
## Kernel version upgrade guide { #compute-instance-kernel-version-upgrade-guide }
...
### Configure default repository { #default-storage-settings }
...
### Clear cache before updating { #clear-the-cache-before-updating }
...
Verify the currently set default kernel.
...
View the list of currently installed kernels.
...
Select one from the list of currently installed kernels and change the default kernel.
...
Reboot the OS to apply the boot order change.
...
### Configure default repository settings { #rocky-linux-9-default-storage-settings }
...
* **name**
  * The name of the repository. You can change this if needed.
* **mirrorlist**
  * Specify a URL that provides a list of mirror servers from which the package can be downloaded.
  * Get a list of mirror servers that match your system's architecture (`$basearch`) and Rocky Linux version (`$releasever`).
* **baseurl**
...
* **enabled**
  ...
  * A value of 1 enables it, and 0 disables it.
* **countme**
...
##### View Currently Installed Kernels
