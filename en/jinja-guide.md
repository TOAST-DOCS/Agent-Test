...
This document is a fixture for verifying that Jinja conditional branching and variable substitution in mkdocs-macros remain identical across three languages (en/en/ja). Tags are control syntax and are not translated; only the body text differs by language. (Body modification test: this sentence should be reflected when translation is re-run.)
