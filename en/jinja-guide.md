...
This document is a fixture to verify that Jinja conditional branching and variable substitution in mkdocs-macros remain identical across three languages (en/en/ja). Tags are control syntax and are not translated; only the content differs by language.
{%- endif %}
...
In the government network environment, use a dedicated endpoint. Public domains are not accessible. (Content modification within tag block: This sentence should be reflected when translation is re-run.)
{% else -%}
