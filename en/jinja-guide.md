...
This document is a fixture for verifying that the Jinja conditional branching and variable substitution in mkdocs-macros are maintained identically across the three languages (ko, en, ja). Tags are control syntax, so they are not translated, and only the body text differs by language. (Body modification test: this sentence should be reflected when the translation is re-executed.)
