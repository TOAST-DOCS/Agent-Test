...
This subsection is a new h3 for validating automatic anchor-id assignment in the translation pipeline. When the Korean version changes, the anchor id is not added manually; instead, the translation job must assign the same id across all three languages (en/en/ja).
...
- **Create an Instance**: Create a virtual server by selecting an image and a type. (test for list modification)
