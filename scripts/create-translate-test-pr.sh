...
#                        * 관찰값 (모델 의존, FAIL 로 쓰지 말 것): 남아있던 두 행이
#                          diff 에 나타나지 않는 것(바이트 보존). 베이스라인은
#                          프롬프트 규칙이라 강제가 아니다. en/ja 기존 문구는
#                          신선 번역이 그대로 재현하기 어렵게 골라 두었으므로
#                          예외가 빠지면 거의 확실히 diff 에 드러나지만, 예외가
#                          있어도 모델이 다시 쓸 수 있다.
#                       실측(2026-08-22, haiku-4-5, 실 API 3회, 요금제 표 167자):
#                       예외 ON = 기존 2행 바이트 보존 3/3, 예외 OFF = 2/3 회에서
