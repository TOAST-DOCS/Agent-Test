...
#                                      [--plan round1|round2|row-drop-repro|llm-patch|table-suite|markup-churn|jinja-mask]
#                                      [--translate api|local]
...
PLAN_NAME="round1"                        # create-translate-test-pr.sh --plan 값. round1|round2|row-drop-repro|llm-patch|table-suite|markup-churn|jinja-mask
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --plan)
      case "${2:-}" in
        round1|round2|row-drop-repro|llm-patch|table-suite|markup-churn|jinja-mask) PLAN_NAME="$2" ;;
        *) echo "error: --plan 은 round1|round2|row-drop-repro|llm-patch|table-suite|markup-churn|jinja-mask 만 지원합니다 (got: ${2:-})" >&2; exit 1 ;;
      esac
      shift 2 ;;
    --translate)
      case "${2:-}" in
